...
export default function StoriesPage() {
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.hero}>
      <p className={styles.eyebrow}>STORIES · 24 HOURS</p>
      <h1>Lightweight proof. Immediate opportunity.</h1>
      <p>Stories disappear after 24 hours. Hustlers can attach a live Service or Product so temporary content can still lead directly to work.</p>
    </section>

    {latestByCreator.length > 0 && <section className={styles.storyRail} aria-label="Active Story creators">
      {latestByCreator.map((story) => {
        const name = story.creator.displayName ?? story.creator.username ?? "Hustler";
    ...
    {!loading && !error && stories.length === 0 && <section className={styles.empty}>
      ...
      <p>Publish the first one and attach a Service or Product if you want viewers to take action.</p>
        ...
        return <a className={styles.card} key={story.id} href={`/stories/${story.id}`}>
          <div className={styles.visual}>
            ...
            {(story.service || story.product) && <div className={styles.attachmentBadge}>
              <small>{story.service ? "SERVICE ATTACHED" : "PRODUCT ATTACHED"}</small>
              <strong>{story.service?.title ?? story.product?.title ?? "Open opportunity"}</strong>
            </div>}
          </div>
